...
aws s3 sync out/_next s3://brace-docs/_next --size-only --cache-control max-age=31536000

aws s3 sync out/nextImageExportOptimizer s3://brace-docs/nextImageExportOptimizer --size-only --cache-control max-age=31536000

aws s3 sync out s3://brace-docs --exclude "_next/*" --exclude "nextImageExportOptimizer/*" --delete --cache-control max-age=86400
